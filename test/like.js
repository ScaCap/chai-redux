...
describe('like', () => {
    it('should wait for all states (partial deep comparison)', done => {
        ...
        expect(store)
            .to.eventually.have.state.like({ value: { firstName: 'Jane', lastName: 'Doe' } })
    ...
    it('should wait for all more complex states (partial deep comparison)', done => {
        ...
        expect(store)
            .to.eventually.have.state.like({
                value: { personalData: { firstName: 'Jane', lastName: 'Doe' } }
            })
            .and.state.like({
                value: { personalData: { firstName: 'Max', lastName: 'Mustermann' } }
            })
            .notify(done);
        store.dispatch({ type: 'TRIGGER' });
        store.dispatch({
    ...
    it('should have all states (partial deep comparison)', () => {
        ...
        store.dispatch({ type: 'LOADING_ERROR' });
        expect(store)
            .to.have.state.like({ value: { firstName: 'Jane', lastName: 'Doe' } })
